...
export default class Fade extends BaseAnimation {
    ...
    public step(params: any[]): void {
        this.hue = (this.hue + Number(params[0])) % 360;
        this.color = hueToRgb(this.hue);

        for (let i = 0; i < this.strip.length; i++) {
            this.strip.setPixelColor(i, this.color.r, this.color.g, this.color.b);
        }
    }
}
